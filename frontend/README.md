...
Frontend работает на React 18 и Next.js 14 App Router. Все точки входа и
маршруты находятся в `app/`, а используемые ими компоненты, Redux-слайсы и
стили — в `src/`. Прежние точки входа Create React App и React Router удалены.

Для установки зависимостей используется npm и зафиксированный
`package-lock.json`. Каталог `frontend/build` — сохранённый артефакт прежней
CRA-схемы; текущие команды его не создают и не используют. Удалять этот каталог
следует только после проверки нового production-сервера по пункту 10 дорожной
карты.
